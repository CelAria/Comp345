...
/*free function to read in map file and parse */ 
  ...
  static void parser(const string& line){
    ...
    while(getline(mystream,line)){
        ...
        //go to next line
         //for every line that isn't "[territories]" AKA WHILE CONTINENTS
          ...
          for(string each; getline(mystream,each, split_char); tokens.push_back(each));{
            name=token[0];
            cout << name << endl;
            points=token[1];
            cout << points << endl;        
        ...
        if(line==territories){
          ...
          vector<string> tokens;
          for(string each; getline(mystream,each, (split_char0 || split_char1 || split_char2 || split_char3 || split_char4 || split_char5 ||split_char6 || split_char7 || split_char8 || split_char9); tokens.push_back(each));{
            countryname =token[0];
            ...
            Country* country;
            Country countryname = new Country(countryname, continentname);
            country =&countryname; 
          //skip all numbers
          ...
          for (int i=2; i < tokenssize(); i++){
            ...
            Country* neighbour;
            Country neighbourcountry = new Country(token[i], continentname);
            countryname.addNeighbour(*neighbour);
